clear;
% switch this to generate the low-resolution data used
% for the image guided upsampling
LOW_RES = 1;
addpath('../simulated_data/nyu_utils');
outdir = './processed'
scenedir = './raw/';
scenes = { 'Reindeer', 'Art', 'Plastic', 'Moebius', 'Laundry', ...
            'Dolls', 'Bowling1', 'Books' };
c = 3e8;
bin_size = 100e-9 / 1024;
num_bins = 1024; 
res = 64;
...
for ss = 1:length(scenes)
    for zz = 1:size(simulation_params,1)
        ...
        if LOW_RES
            % for upsampling, fill in the holes
            mask = disparity == dmin;
            depth(mask) = nan;

            % manually fill in holes in the bowling scene
            if ss == 7
                mask = disparity == dmin;
                se = strel('disk',3);
        ...
        spad = reshape(spad, size(spad,1)*size(spad,2), []);
        spad = sparse(spad); 
        ...
        if LOW_RES
            ...
            save(out_fname, 'spad', 'depth', 'SBR', 'mean_signal_photons', 'mean_background_photons', 'bin_size','intensity','intensity_hr', 'depth_hr');

        else
            out_fname = sprintf('%s/%s_%s_%s.mat',outdir, scenes{ss}, num2str(mean_signal_photons), num2str(mean_background_photons));
            save(out_fname, 'spad', 'depth', 'SBR', 'mean_signal_photons', 'mean_background_photons', 'bin_size','intensity');
        end
    end
end
