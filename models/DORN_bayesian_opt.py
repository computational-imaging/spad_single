...
class DORN_bayesian_opt:
    ...
    def initialize(self, input_, device):
        ...
        with torch.no_grad():
            ...
            # Start with log probs, add a small epsilon to prevent -inf
            # depth_hist
        return depth
...
if __name__ == "__main__":
    ...
    model = DORN_bayesian_opt(sgd_iters=sgd_iters, use_albedo=spad_config["use_albedo"],
                              use_squared_falloff=spad_config["use_squared_falloff"], lr=1e-4)
    ...
    depth = model.initialize(input_, device)
    depth_optimized = model.optimize_depth_map(depth, input_, device)

    # Decode the output using usual ordinal decoding method.
    # Normalize each pixel histogram
    depth_optimized = depth_optimized / torch.sum(depth_optimized, dim=1, keepdim=True)

    plt.figure()
    plt.plot(depth_optimized[:,:,130,130].squeeze().clone().detach().cpu().numpy())
    plt.title("(130, 130) histogram at end")
    plt.draw()
    plt.pause(0.001)

    logprobs = torch.log(1. - torch.cumsum(depth_optimized, dim=1))
    depth_pred = model.ord_decode((logprobs,None), model.sid_obj)
