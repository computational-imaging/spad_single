...
class DORN_nyu_rawhints(nn.Module):
    """
    Deep Ordinal Regression Network with Raw Depth Hints

    Uses DORN_nyu, ported from caffe.

    Only meant to be run in inference mode.
    """
    def __init__(self, in_channels=3, in_height=257, in_width=353, sid_bins=136,
                 hist_bins=136, alpha = 1., frozen=True, pretrained=True,
                 state_dict_file="torch_params_nyuv2_BGR.pth.tar", **kwargs):
        super(DORN_nyu_rawhints, self).__init__()
        self.in_channels = in_channels
        self.in_height = in_height
        self.in_width = in_width
        self.sid_bins = sid_bins
        self.hist_bins = hist_bins
        self.alpha = alpha # Amount to weight the hints over the pixel probs.
        self.frozen = frozen
        self.pretrained = pretrained
        self.state_dict_file = state_dict_file
        self.feature_extractor = DORN_nyu(in_channels, in_height, in_width, sid_bins,
                                          frozen, pretrained, state_dict_file, **kwargs)

    def forward(self, input_):
        rgb = input_["rgb"]
        sid_hist = input_["sid_hist"]
        sid_hist_cdf = torch.cumsum(sid_hist, dim=1)
        log_probs = self.feature_extractor(rgb)
        # cdf: P(l <= k)
        cdf = sid_hist_cdf.expand(-1, -1, log_probs.size(2), log_probs.size(3))
        # cdf complement: 1 - P(l <= k) = P(l > k)
        cdf_comp = 1. - cdf
        # Odd indices correspond to P(depth > k)
        # log_ord_c0 = log_probs[:, 1::2, :, :] + cdf_comp
        # log_ord_c1 = log_probs[:, ::2, :, :] + cdf
        log_probs[:, 1::2, :, :].add_(self.alpha * cdf_comp)
        log_probs[:, ::2, :, :].add_(self.alpha * cdf)
        return log_probs
