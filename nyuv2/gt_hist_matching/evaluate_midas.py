...
ex = Experiment("midas_gt_hist_matching", ingredients=[nyuv2_labeled_ingredient])
...
def cfg(data_config):
    ...
    seed = 95290421
    small_run = 0
    input_file = os.path.join("data", "midas_{}_outputs.npy".format(dataset_type))
...
def run(dataset_type,
    ...
    if entry is None:
        ...
        if save_outputs:
            np.save(os.path.join(output_dir, "midas_gt_hist_{}_outputs.npy".format(dataset_type)),
        ...
        metrics_df.to_pickle(path=os.path.join(output_dir, "midas_gt_hist_{}_metrics.pkl".format(dataset_type)))
        ...
        average_df.to_csv(os.path.join(output_dir, "midas_gt_hist_{}_avg_metrics.csv".format(dataset_type)), header=True)
