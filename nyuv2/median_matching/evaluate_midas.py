...
ex = Experiment("midas_median_rescaling", ingredients=[nyuv2_labeled_ingredient])
...
def run(dataset_type,
    ...
    if entry is None:
        ...
        if save_outputs:
            np.save(os.path.join(output_dir, "midas_median_{}_outputs.npy".format(dataset_type)),
        ...
        metrics_df.to_pickle(path=os.path.join(output_dir, "midas_median_{}_metrics.pkl".format(dataset_type)))
        ...
        average_df.to_csv(os.path.join(output_dir, "midas_median_{}_avg_metrics.csv".format(dataset_type)), header=True)
