...
%%%%%%%%% ABSTRACT
\begin{abstract}
Existing monocular depth estimation algorithms do an outstanding job of predicting the relative depth {order} of objects in a scene.
However, because of the fundamental scale ambiguity associated with monocular images, these algorithms do a poor job at predicting an object's true metric depth. 
In this work, we demonstrate how depth histograms, which can be readily captured using single photon avalanche diodes (SPADs), can be fused with existing monocular depth estimation algorithms to resolve the depth ambiguity problem. 
We validate this novel sensor fusion technique experimentally and in extensive simulation and show that it dramatically improves the performance of several state-of-the-art monocular depth estimation algorithms.
\end{abstract}
