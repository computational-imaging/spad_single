...
zero for all bins where $n < n_\text{first}$ and $n > n_\text{last}$. We use 
...
matching procedure, reducing computation time while maintaining accuracy. The
value of $K$ is selected to trade off between speed and accuracy. For
the simulated results we use $K = 140$ bins with $(\ell,
...
to the histogram bin. We also re-bin this histogram, again following Fu et al.\
