In this section, we describe the image formation of a diffused pulsed laser and time-resolved detector. \edit{Although our model is derived for the specific case of imaging with a single-photon avalanche diode (SPAD), the resulting image formation model equally applies to other time-resolved detectors}. We also describe an approach for correcting a depth map generated with a monocular depth estimator to match the global scene information captured by the \edit{transient}.
...
\begin{figure}[t!]
  ...
  \caption{Overview of processing pipeline. The processing pipeline uses
      the input \edit{transient measurement} and an RGB image to produce an accurate depth
      map. The \edit{transient is} preprocessed to adjust for ambient photon
      ...
      depth histogram is compared to the processed \edit{transient} to
...
the repetition rate of the laser. \edit{As derived in previous work, the resulting measurement
...
count}. \edit{Like previous work, we neglect scene interreflections
...
\edit{For a moderate number of background counts, each background bin $h[n]$ can be
...
zero for all bins where \edit{$n < \hat n_\text{first}$} and $n > \hat n_\text{last}$. We use 
...
\end{equation}
%
This rebinning procedure allows us to use a reduced number of bins by the histogram
matching, reducing computation time while maintaining accuracy.
\edit{For the simulated results we use $K = 140$ bins with $(\ell,
...
to the histogram bin. \edit{We also re-bin this histogram, following Fu et al. and using $K = 140$ with $(\ell, u) = (0.657, 9.972)$~\cite{Fu2018}.}
...
moved to $h_\text{target}[n]$. \edit{We refer the reader to the supplement for
...
\edit{Pseudo-code for this procedure is included in the supplement; we will make source code and data available.}
