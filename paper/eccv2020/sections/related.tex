...
networks have been applied to this problem, \edit{for example using a multi-scale neural network to predict depth maps~\cite{Eigen2014},
...
histogram--like measurements obtained from a single \edit{time-resolved detector}. Other
...
and specialized image reconstruction methods. \edit{In contrast, our approach adds
...
%
\edit{Emerging LiDAR systems use avalanche photon diodes (APDs) or single-photon avalanche diodes (SPADs)
...
cost. For this reason, many \edit{LiDAR systems}, especially those using SPADs, use a single \edit{or a few detectors}
...
to mechanical failure. To reduce the scanning complexity to one dimension, \edit{1D
...
single-pixel \edit{time-resolved detectors} remain the only viable option for low-cost consumer devices
today.


The proposed method uses a single-pixel \edit{APD or SPAD} and pulsed light source that are
...
reliable absolute depth estimation by combining the \edit{transient} measurement with the
...
histograms. \edit{Caramazza et al.~\cite{caramazza2018neural} use a single
...
In our procedure, the \edit{transient measurements} closely resemble a histogram of
