...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.3\columnwidth]{teaser.pdf}
  \caption{Monocular depth estimation predicts a depth map (upper right) from a
    ...
    The proposed method uses a \edit{single transient measurement aggregating the time-of-flight information of the entire scene (lower left)} to correct the output of the depth estimation and optimize the quality of the
...
\edit{
Time-resolved detectors, such as avalanche photon diodes (APDs) or single-photon avalanche diodes (SPADs), measure the full waveform of time-resolved incident radiance at each pixel. These detectors form the backbone of modern LiDAR
systems~\cite{Kirmani:2014,Li:2019,pawlikowska2017single}. However, single-photon sensor arrays} have
...
entire scene with the detector}. The resulting \edit{transient} measurement resembles a histogram of the depth of
the scene and we demonstrate that this can be used to achieve accurate absolute
depth when combined with the estimate of \edit{any monocular depth estimator in a post-processing step} (\cf~Figure~\ref{fig:teaser}).
...
used. \edit{Moreover, we build an camera prototype that combines an RGB camera and a single-pixel time-resolved detector.}  With this
...
%
\begin{itemize}
	\item We propose augmenting an RGB camera with a global depth histogram \edit{aggregated by a diffused time-resolved detector} to address scale ambiguity error in monocular depth estimators.	
...
\edit{\emph{Overview of Limitations:} our prototype camera uses a scanned SPAD and digitally aggregates the captured transients to emulate a single diffused measurement. The benefit of this approach is that we have access to ground truth depth, allowing us to evaluate the efficacy of our method with measured data. However, when operating SPADs in certain conditions, they may observe a nonlinear aggregation effect known as pileup. In these conditions, the emulated measurements may differ from a single optically diffused measurement. Yet, we experimentally verify that digitally and optically aggregated measurements captured with our system are very similar and pileup could also be computationally corrected~\cite{Heide:2018,Rapp:2019}, although we did not attempt to do this.} 
